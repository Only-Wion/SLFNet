...
############################## 从 configure 中导入参数配置 ##############################
import yaml
parser = argparse.ArgumentParser(description="SLFNet train/test")
parser.add_argument("--config", default="./configure/kitti_cfg.yaml",
                    help="Path to yaml config file")
cli_args = parser.parse_args()
args = yaml.safe_load(open(cli_args.config, 'r'))
...
def model_test():
    ...
    for batch_idx, data in enumerate(TestImgLoader):
        if args["test"].get("max_samples") is not None and batch_idx >= args["test"]["max_samples"]:
            break
